...
uniform sampler2D blenMapTexture;
...
vec3 calculateDirectionalLight(Light light, vec3 direction){
	...
	vec4 blenMapColor = texture(blenMapTexture, our_uv);
	float backTextureColor = 1-(blenMapColor.r+blenMapColor.g+blenMapColor.b);
	vec4 backgroundColor = texture(backgroundTexture, tiledCoords) * backTextureColor;
	vec4 rColor = texture(rTexture, tiledCoords)* blenMapColor.r;
	vec4 gColor = texture(gTexture, tiledCoords)* blenMapColor.g;
	vec4 bColor = texture(bTexture, tiledCoords)* blenMapColor.b;
