...
vec3 calculateDirectionalLight(Light light, vec3 direction){
	...
	vec4 colorText3 = mix(colorText1,colorText2,0.5);
	vec3 ambient  = light.ambient * vec3(colorText3);
  	
    // Diffuse 
    vec3 normal = normalize(our_normal);
    vec3 lightDir = normalize(-direction);
    float diff = max(dot(normal, lightDir), 0.0);
    vec3 diffuse  = light.diffuse * (diff * vec3(colorText3));//vec3(texture(texture1, our_uv)));
    ...
    vec3 specular = light.specular * (spec * vec3(colorText3));//vec3(texture(texture1, our_uv)));
...
void main()
{
	vec4 colorText3 = texture(texture1,our_uv);
	if(colorText3.a < 0.1)
		discard;
    color = vec4(calculateDirectionalLight(directionalLight.light, directionalLight.direction) + calculatePointLights() + calculateSpotLights(), colorText3.a);
}