...
bool testRayOBB(glm::vec3 o,glm::vec3 t, AbstractModel::OBB collider){
	...
	glm::vec3 oInv = qInv * o;// transformar el origen, destino y centro de la caja con el inverso
	glm::vec3 tInv = qInv * t;
	glm::vec3 cOBBInv = qInv * collider.c;
	AbstractModel::AABB colliderAABB;
	colliderAABB.maxs = cOBBInv + collider.e;//crear un collider AABB
	colliderAABB.mins = cOBBInv - collider.e;
	return intersectSegmentAABB(oInv,tInv, colliderAABB);// utilizar la funcion de colision con un rayo con una caja alineada
